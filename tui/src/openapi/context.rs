...
const DEEPBOOK_CONFIG_DIR: &str = ".deepbook";
const DEEPBOOK_CONFIG_FILE: &str = "config.json";
...
impl QuoteContext {
    fn new(stream_tx: mpsc::UnboundedSender<PushEvent>) -> Result<Self> {
        let file_config = load_deepbook_config();
        let network = file_config
            .as_ref()
            .and_then(|config| config.network.as_deref())
            .unwrap_or("mainnet");
        let surflux_config = file_config
            .as_ref()
            .and_then(|config| config.providers.as_ref())
            .and_then(|providers| providers.surflux.as_ref());

        let rest_base_url = env_or_config_url(
            "SURFLUX_API_BASE_URL",
            surflux_config.and_then(|config| config.base_url_for(network)),
            DEFAULT_REST_BASE_URL,
        );
        let stream_base_url = env_or_config_url(
            "SURFLUX_STREAM_BASE_URL",
            surflux_config.and_then(|config| config.stream_base_url_for(network)),
            DEFAULT_STREAM_BASE_URL,
        );

        let mut pool_key_map = discover_pool_keys(STREAM_KEY_SUFFIX);
        if let Some(config) = surflux_config {
            for (symbol, key) in config.stream_api_key_map() {
                pool_key_map.entry(symbol).or_insert(key);
            }
        }
        if pool_key_map.is_empty() {
            return Err(anyhow!(
                "No stream keys found. Set BASE_QUOTE_STREAM_API_KEY (for example: SUI_USDC_STREAM_API_KEY) or configure deepbook config set-stream-key."
            ));
        }
        ...
        let rest_default_key = std::env::var(REST_KEY_ENV)
            ...
            .or_else(|| surflux_config.and_then(SurfluxFileConfig::read_api_key));
...
struct DeepbookFileConfig {
    #[serde(default)]
    network: Option<String>,
    #[serde(default)]
    providers: Option<DeepbookProviderConfig>,
}

#[derive(Debug, Deserialize, Default)]
struct DeepbookProviderConfig {
    #[serde(default)]
    surflux: Option<SurfluxFileConfig>,
}

#[derive(Debug, Deserialize, Default)]
struct SurfluxFileConfig {
    #[serde(default, rename = "readApiKey")]
    read_api_key: Option<String>,
    #[serde(default, rename = "streamApiKeys")]
    stream_api_keys: HashMap<String, String>,
    #[serde(default, rename = "baseUrls")]
    base_urls: HashMap<String, String>,
    #[serde(default, rename = "streamBaseUrls")]
    stream_base_urls: HashMap<String, String>,
}

impl SurfluxFileConfig {
    fn read_api_key(&self) -> Option<String> {
        non_empty(self.read_api_key.as_deref()).map(str::to_string)
    }

    fn stream_api_key_map(&self) -> HashMap<String, String> {
        self.stream_api_keys
            .iter()
            .filter_map(|(symbol, key)| {
                let key = non_empty(Some(key.as_str()))?;
                Some((normalize_symbol(symbol), key.to_string()))
            })
            .collect()
    }

    fn base_url_for(&self, network: &str) -> Option<&str> {
        config_url_for_network(&self.base_urls, network)
    }

    fn stream_base_url_for(&self, network: &str) -> Option<&str> {
        config_url_for_network(&self.stream_base_urls, network)
    }
}

fn load_deepbook_config() -> Option<DeepbookFileConfig> {
    let path = deepbook_config_path()?;
    if !path.exists() {
        return None;
    }
    ...
    match fs::read_to_string(&path)
        ...
        .and_then(|raw| {
            serde_json::from_str::<DeepbookFileConfig>(&raw)
                .with_context(|| format!("failed to parse {}", path.display()))
        }) {
        Ok(config) => Some(config),
        Err(err) => {
            tracing::warn!("Ignoring DeepBook CLI config: {err}");
            None
        }
    }
}

fn deepbook_config_path() -> Option<PathBuf> {
    Some(
        dirs::home_dir()?
            .join(DEEPBOOK_CONFIG_DIR)
            .join(DEEPBOOK_CONFIG_FILE),
    )
}

fn env_or_config_url(env_name: &str, config_value: Option<&str>, default_value: &str) -> String {
    std::env::var(env_name)
        .ok()
        .and_then(|value| non_empty(Some(value.as_str())).map(str::to_string))
        .or_else(|| config_value.and_then(|value| non_empty(Some(value)).map(str::to_string)))
        .unwrap_or_else(|| default_value.to_string())
}

fn config_url_for_network<'a>(urls: &'a HashMap<String, String>, network: &str) -> Option<&'a str> {
    urls.get(network)
        .or_else(|| urls.get(&network.to_ascii_lowercase()))
...
pub fn print_config_guide() {
    ...
    eprintln!();
    eprintln!("DeepBook CLI config fallback:");
    eprintln!("  deepbook config set-read-key");
    eprintln!("  deepbook config set-stream-key SUI_USDC");
    eprintln!("  deepbook config set-provider-base-url mainnet https://api.surflux.dev");
    eprintln!("  deepbook config set-provider-stream-base-url mainnet https://flux.surflux.dev");
}
